...
#convert the labels (or scores) into a binary array based on a chosen number of bins
#score = actual score (or other target feature value) from the database
#num_classes = number of labels to be used- dictates size of the binary array
#max_score = ceiling of the scores so that the intervals can be calculated given the number of classes
#assumed min score of 0
...
#convert the labels (that are strings) into a binary array based on number of unique items for that feature
#item = value from the database
#type_range = list of all unique values for the feature
